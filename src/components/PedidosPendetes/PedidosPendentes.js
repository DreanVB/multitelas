      ...
      <div className="row">
        ...
        <div className="col-md-6">
          ...
          <table className="table table-striped">
            ...
            <tbody>
              {(() => {
                if (!dados?.day) return null;

                // 1. Agrupar por data, considerando filtro
                const documentosFiltrados = dados.day.filter(item => filterBySearch(item, false));

                const documentosPorData = documentosFiltrados.reduce((acc, item) => {
                  const data = new Date(item.DTPREVISAO);
                ...
                const diasOrdenados = Object.keys(documentosPorData).sort();

                return diasOrdenados.map((data, i) => (
                  <React.Fragment key={i}>
                    {(() => {
                      // 1) Pega os itens já ordenados
                      const ordenados = documentosPorData[data].sort((a, b) => {
                        const hoje = new Date().toISOString().slice(0, 10);
                        if (data === hoje) {
                          // hoje -> ordena por HORAPREVISAO
                          const toMinutes = item => {
                            const h = item.HORAPREVISAO?.slice(0, 2) || '00';
                            const m = item.HORAPREVISAO?.slice(2) || '00';
                            return parseInt(h, 10) * 60 + parseInt(m, 10);
                          };
                          return toMinutes(a) - toMinutes(b);
                        } else {
                          // outros dias -> usa DTPREVISAO completa
                          return new Date(a.DTPREVISAO) - new Date(b.DTPREVISAO);
                        }
                      });

                      // 2) Remove duplicados e monta um novo array
                      const vistos = new Set();
                      const unicos = [];
                      for (const item of ordenados) {
                        if (!vistos.has(item.DOCUMENTO)) {
                          vistos.add(item.DOCUMENTO);
                          unicos.push(item);
                        }
                      }
                      ...
                      // 3) Agora mapeia os únicos e usa o índice para numeração certinha
                      return filtrados.map((item, idx) => (
                        <tr
                          key={item.DOCUMENTO}
                          onDoubleClick={() => setSearchTerm(String(item.DOCUMENTO))}
                        ...
                        >
                          ...
                          <td>{(() => {
                            ...
                            return situacao.text.includes(':')
                              ? situacao.text.split(' ')[1]
                              : `${situacao.horaFormatada}:${situacao.minutoFormatado}`;
                          })()}</td>
                          <td style={{ color: getSituacaoEstilo(item).color }}>
                            {(() => {
                              const situacao = getSituacaoEstilo(item);
                              return situacao.text.includes(':')
                                ? situacao.text.split(' ')[0]
                                : situacao.text;
                            })()}
                          </td>
                        </tr>
